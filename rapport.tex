...
\vhEntry{0.1.7}{20.11.21}{Bourgeois Noé }{Lancement, enregistrement, créer ou rejoindre une partie, rencontre inopinée de [alex ellioti]? sur overleaf}
...
Si il le désire, l'utilisateur pourra demander de l'aide, l'application affichera le fonctionnement du jeu et les différentes fonctionnalités de cette dernière.
...
Toutes les actions doivent passer d’un utilisateur à un autre avec une latence minimale pour avoir l’air instantanée. Ceci dit, c'est un paramètre qui dépend énormément de la qualité internet des joueurs participants.   %le jeux se fait en local ou en ligne ?
\subsection{Légalité}
...
"new game", une
fenêtre de configuration de jeu apparaît contenant les paramètres par défaut: \\
...
\\-Si c'est un draw, les scores ne changent pas.
...
\[
 \frac{\text{taille du plateau + le nombre de murs}}{\text{nombre de tours}}
\]
